...
function MultiPartySignature({
  ...
  const validateSignature = (partyId) => {
    ...
    if (!hasReadAgreement[partyId]) {
      newErrors[`${partyId}-agreement`] =
        'Party must read the complete agreement';
    }

    if (!agreementAccepted[partyId]) {
      newErrors[`${partyId}-acceptance`] =
        'Party must accept the agreement terms';
    }
      ...
      <div className="space-y-8">
        <div className="text-center">
          ...
          <p className="mt-4 text-lg text-gray-600 max-w-2xl mx-auto">
            {party?.name} ({party?.role}) must review and sign the credit
            agreement.
          </p>
        </div>
        ...
        <div className="card">
          ...
          <div className="space-y-4">
            <div className="flex items-start space-x-3">
              <input
                ...
                onChange={(e) => {
                  ...
                  if (errors[`${currentSigningParty}-acceptance`]) {
                    setErrors((prev) => ({
                      ...prev,
                      [`${currentSigningParty}-acceptance`]: undefined,
                    }));
                  }
                }}
              ...
              />
              <label
                htmlFor={`agreementAccepted-${currentSigningParty}`}
                className={`text-sm ${
                  !hasReadAgreement[currentSigningParty]
                    ? 'text-gray-400'
                    : 'text-gray-700'
              ...
              >
                I, {party?.name}, have read, understood, and agree to all terms
                and conditions of this Credit Agreement in my capacity as{' '}
                {party?.role}. I acknowledge that this agreement is legally
                binding and enforceable.
              </label>
            </div>
            {errors[`${currentSigningParty}-acceptance`] && (
              <p className="error-text ml-7">
                {errors[`${currentSigningParty}-acceptance`]}
              </p>
            )}
          </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (errors[`${currentSigningParty}-signature`]) {
                    setErrors((prev) => ({
                      ...prev,
                      [`${currentSigningParty}-signature`]: undefined,
                    }));
                  }
                }}
              ...
              <p className="text-xs text-gray-500 mt-1">
                ...
                electronic signature as {party?.role}
              </p>
            </div>
        ...
        <div className="flex justify-between">
          ...
          <button
            ...
            disabled={
              !hasReadAgreement[currentSigningParty] ||
              !agreementAccepted[currentSigningParty] ||
              !signatureData.trim() ||
              isSigning
            }
  ...
  return (
    <div className="space-y-8">
      <div className="text-center">
        ...
        <p className="mt-4 text-lg text-gray-600 max-w-2xl mx-auto">
          All{' '}
          {applicantType === 'partnership' ? 'partners' : 'authorized parties'}{' '}
          must provide their electronic signature to make this credit agreement
          legally binding.
        </p>
      </div>
      ...
      <div className="bg-blue-50 rounded-lg p-6">
        ...
        <div className="w-full bg-blue-200 rounded-full h-3">
          <div
            ...
            style={{
              width: `${
                stats.total > 0 ? (stats.signed / stats.total) * 100 : 0
              }%`,
            }}
          />
        </div>
      ...
      <div className="flex justify-between">
        ...
        <button disabled={!allSigned} className="btn-primary px-8 py-3">
