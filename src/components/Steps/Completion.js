    ...
    switch (productId) {
      case 'term-loan':
        return {
          ...
          description: 'Funds will be disbursed to your selected business account within 2-3 business days.',
          action: 'Check your business account for the loan amount'
        };
      case 'green-loan':
        return {
          title: 'Green Loan Disbursement',
          description: 'Your environmentally-focused loan funds will be available in your account within 2-3 business days.',
          action: 'Funds are earmarked for your approved green purpose'
        };
      case 'business-overdraft':
        return {
          title: 'Overdraft Facility Active',
          description: 'Your overdraft limit is now available in your designated business account.',
          action: 'You can access funds immediately as needed'
        };
      case 'business-credit-card':
        return {
          title: 'Credit Card Dispatch',
          description: 'Your business credit card will be dispatched to your registered address within 5-7 business days.',
...
Applicant Type: ${state.applicantType?.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase())}
...
${state.selectedProducts?.map(product => 
  `- ${product.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase())}`
).join('\n') || 'None'}

DECISION
Status: ${state.decisionOutcome?.type?.toUpperCase() || 'PENDING'}
${state.decisionOutcome?.type === 'approved' ? 'Amount Approved: ' + formatCurrency(state.applicationDetails?.amountRequested || 0) : ''}

${state.creditAgreement?.signed ? 'AGREEMENT SIGNED: ' + new Date(state.creditAgreement.signedDate).toLocaleDateString('en-IE') : ''}
    ...
    <div className="space-y-8">
        ...
        <div className={`p-4 rounded-lg border-l-4 ${
          ...
          <div className="flex items-center">
            <div className={`flex-shrink-0 ${
              state.decisionOutcome?.type === 'approved' ? 'text-green-400' :
              state.decisionOutcome?.type === 'pending' ? 'text-yellow-400' :
              'text-red-400'
            }`}>
            ...
            <div className="ml-3">
              <h3 className={`text-lg font-medium ${
                state.decisionOutcome?.type === 'approved' ? 'text-green-800' :
                state.decisionOutcome?.type === 'pending' ? 'text-yellow-800' :
                'text-red-800'
              }`}>
                {state.decisionOutcome?.title}
              </h3>
              <p className={`text-sm ${
                state.decisionOutcome?.type === 'approved' ? 'text-green-700' :
                state.decisionOutcome?.type === 'pending' ? 'text-yellow-700' :
                'text-red-700'
              }`}>
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Contact your relationship manager to set up automatic repayments for your loan facilities.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                All documentation and next steps will be sent to your registered email address.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              <p className="font-medium text-gray-900">Customer Support Available</p>
              <p className="text-sm text-gray-600">
                Contact us at +353 1 234 5678 if you have any questions about your application.
              </p>
            </div>
          </div>
          ...
          {state.creditAgreement?.signed && (
            <div className="flex items-start space-x-3">
              ...
              <div>
                <p className="font-medium text-gray-900">Download Your Documents</p>
                <p className="text-sm text-gray-600">
                  Your signed credit agreement and application summary are available for download.
                </p>
              </div>
            </div>
          )}
      ...
      <div className="bg-gray-50 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <p className="text-gray-600">Email: support@smebanking.ie</p>
            <p className="text-gray-600">Hours: Mon-Fri 9AM-5PM</p>
          </div>
          
          <div>
            <p className="font-medium text-gray-900">Online Banking</p>
            <p className="text-gray-600">Website: www.smebanking.ie</p>
            <p className="text-gray-600">Mobile App: Available on iOS & Android</p>
            <p className="text-gray-600">24/7 Account Access</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-blue-50 rounded-lg p-6">
        ...
        <ul className="text-sm text-blue-700 space-y-1">
          <li>• SME Banking Solutions is regulated by the Central Bank of Ireland</li>
          <li>• Member of the Deposit Guarantee Scheme</li>
          <li>• Complaints can be made to the Financial Services and Pensions Ombudsman</li>
          <li>• All credit decisions comply with Irish and EU lending regulations</li>
          <li>• Your data is protected under GDPR and processed securely in Ireland</li>
        </ul>
      </div>
      ...
      <div className="text-center py-8">
        ...
        <p className="text-gray-600 max-w-2xl mx-auto">
          We appreciate your business and look forward to supporting your company's growth and success. 
          Your trust in us is valued, and we're committed to providing excellent service.
        </p>
      </div>
    </div>
  );
