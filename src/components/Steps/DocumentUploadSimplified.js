  ...
  'all': [
    {
      id: 'business-plan',
      name: 'Business Plan',
      description: 'Current business plan or strategy document',
    ...
    },
    {
      ...
      description: 'Latest management accounts and cash flow projections',
      icon: DocumentTextIcon,
      required: true
    }
  ],
...
const mockDocumentContent = {
  'business-plan': 'Executive Summary: TechGreen Solutions is a growing SME specializing in sustainable technology solutions...',
  'financial-statements': 'INCOME STATEMENT - Year Ending 2023\nRevenue: €2,450,000\nCost of Goods Sold: €1,470,000\nGross Profit: €980,000...',
  'bank-statements': 'BUSINESS ACCOUNT STATEMENT - November 2024\nOpening Balance: €245,670\nTotal Credits: €387,420\nTotal Debits: €298,150...',
  'management-accounts': 'MANAGEMENT ACCOUNTS - Q3 2024\nOperating Revenue: €612,000\nOperating Expenses: €456,000\nEBITDA: €156,000...',
  'environmental-impact': 'ENVIRONMENTAL IMPACT ASSESSMENT\nProject: Solar Panel Installation\nCO2 Reduction: 45 tonnes annually...',
  'asset-valuation': 'ASSET VALUATION REPORT\nEquipment: Manufacturing Line\nCurrent Market Value: €485,000...',
  'purchase-agreement': 'EQUIPMENT PURCHASE AGREEMENT\nVendor: GreenTech Manufacturing\nTotal Cost: €125,000...',
  'green-certification': 'ISO 14001 ENVIRONMENTAL MANAGEMENT CERTIFICATION\nCertified Company: TechGreen Solutions...',
  'energy-audit': 'ENERGY AUDIT REPORT 2024\nCurrent Annual Consumption: 145,000 kWh\nPotential Savings: 35%...'
};
...
function DocumentUploadSimplified() {
  ...
  const handleDocumentUpload = (documentId) => {
    ...
    const mockDocument = {
      ...
      uploadDate: new Date(),
      status: 'uploaded',
      content: mockDocumentContent[documentId] || 'Document content would appear here...'
    };
  ...
  const getAIAnalysis = (documentId) => {
    const analyses = {
      'business-plan': ['Strong revenue projections', 'Clear market strategy', 'Experienced management team'],
      'financial-statements': ['Healthy profit margins', 'Strong cash position', 'Low debt-to-equity ratio'],
      'bank-statements': ['Consistent cash flow', 'Regular customer payments', 'Good banking history'],
      'management-accounts': ['EBITDA trending upward', 'Cost control improving', 'Revenue growth 15% YoY'],
      'environmental-impact': ['Significant CO2 reduction', 'Measurable environmental benefits', 'Compliance with regulations'],
      'asset-valuation': ['Fair market valuation', 'Assets in good condition', 'Adequate collateral coverage'],
      'purchase-agreement': ['Competitive pricing', 'Reputable vendor', 'Standard terms and conditions'],
      'green-certification': ['Valid certifications', 'Environmental compliance', 'Sustainability commitment'],
      'energy-audit': ['High efficiency potential', 'Quick payback period', 'Substantial energy savings']
    };
    return analyses[documentId] || ['Document processed successfully', 'Information extracted', 'Ready for review'];
  };

  const getAISuggestions = (documentId) => {
    const suggestions = {
      'business-plan': 'Consider adding more specific market share targets for the next 3 years.',
      'financial-statements': 'Strong financials support your loan application. Consider highlighting cash flow stability.',
      'bank-statements': 'Excellent banking history. This strengthens your creditworthiness significantly.',
      'management-accounts': 'Recent performance trends are very positive for loan approval.',
      'environmental-impact': 'Environmental benefits exceed minimum requirements for green loan qualification.',
      'asset-valuation': 'Asset values provide excellent security for the requested loan amount.',
      'purchase-agreement': 'Purchase terms are favorable and support the business case.',
      'green-certification': 'Certifications qualify you for preferential green loan rates.',
      'energy-audit': 'Energy savings projections support the loan business case strongly.'
    };
    return suggestions[documentId] || 'Document meets all requirements for loan application.';
  };
    ...
    <div className="space-y-8">
      <div className="text-center">
        ...
        <p className="mt-4 text-lg text-gray-600 max-w-2xl mx-auto">
          Upload your business documents for AI-powered analysis and instant processing.
        </p>
        
        <div className="mt-6 flex justify-center">
          <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-2">
            <span className="text-sm font-medium text-blue-800">
              Progress: {uploadedCount} of {requiredCount} required documents uploaded
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-lg p-6">
        <div className="flex items-start space-x-3">
          ...
          <div>
            <h3 className="text-lg font-medium text-purple-800 mb-2">AI-Powered Document Processing</h3>
            <div className="text-sm text-purple-700 space-y-1">
              <p>• <strong>Instant Analysis:</strong> Documents are analyzed immediately upon upload</p>
              <p>• <strong>Key Information Extraction:</strong> Important financial metrics and data are automatically identified</p>
              <p>• <strong>Compliance Checking:</strong> Documents are verified against regulatory requirements</p>
              <p>• <strong>Risk Assessment:</strong> AI evaluates creditworthiness and application strength</p>
            </div>
          </div>
        </div>
      </div>
            ...
            >
              ...
              {!isUploaded ? (
                ...
                >
                  ...
                  <span className="text-xs text-gray-500 mt-1">PDF, DOC, XLS (Max 10MB)</span>
                </button>
              ) : (
                <div className="space-y-3">
                  ...
                  <div className="bg-white rounded-md p-3 border">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium text-gray-900">{isUploaded.name}</span>
                      <span className="text-xs text-gray-500">{formatFileSize(isUploaded.size)}</span>
                    </div>
                    ...
                    {editingDocument === document.id ? (
                      <div className="space-y-3">
                        <textarea
                          ...
                          onChange={(e) => setDocumentContent(prev => ({ ...prev, [document.id]: e.target.value }))}
                          className="w-full h-24 text-xs border rounded px-2 py-1 resize-none"
                          placeholder="Document content..."
                        />
                        <div className="flex space-x-2">
                          <button
                            onClick={() => handleSaveEdit(document.id, documentContent[document.id])}
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <span className={`text-xs px-2 py-1 rounded ${
                        ...
                        isEdited ? 'bg-blue-100 text-blue-800' :
                        'bg-yellow-100 text-yellow-800'
                      }`}>
                        {isProcessed ? 'AI Processed' : isEdited ? 'Edited' : 'Processing...'}
                      </span>
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-md p-3">
                      <div className="flex items-center mb-2">
                        ...
                        <span className="text-xs font-medium text-blue-800">
                          AI Analysis ({isUploaded.aiAnalysis.confidence}% confidence)
                        </span>
                      </div>
                      
                      <div className="space-y-2">
                        <div>
                          <div className="text-xs font-medium text-blue-700 mb-1">Key Findings:</div>
                          <ul className="text-xs text-blue-600 space-y-0.5">
                            {isUploaded.aiAnalysis.keyFindings.map((finding, index) => (
                        ...
                        <div>
                          <div className="text-xs font-medium text-blue-700 mb-1">AI Suggestion:</div>
                          <p className="text-xs text-blue-600">{isUploaded.aiAnalysis.suggestions}</p>
                        </div>
                      </div>
                    </div>
                  )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600">
                {Object.values(uploadedDocuments).filter(doc => doc.status === 'processed').length}
              </div>
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600">
                {Math.round(Object.values(uploadedDocuments).reduce((acc, doc) => 
                  acc + (doc.aiAnalysis?.confidence || 0), 0) / Object.keys(uploadedDocuments).length) || 0}%
              </div>
